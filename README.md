...
Создайте интерфейс репозитория для сущности Book, используя Spring Data JDBC.
...
Department должна содержать следующие поля: id (автоматически генерируемый идентификатор), name (название отдела). Репозитории:
...
## task03:
### Практическое задание - Spring data - Transactional

Задача: Разработать систему учета заказов в интернет-магазине с использованием Spring и обеспечить консистентность данных с помощью вложенных транзакций.
Требования:

Сущности:

Создайте три сущности: Customer, Order, и Product.

Customer должна содержать следующие поля:

id (автоматически генерируемый идентификатор),

username (имя пользователя),

balance (баланс средств на счету).

Order должна содержать следующие поля:

id (автоматически генерируемый идентификатор),

customer (ссылка на заказчика),

products (ссылки на продукты в рамках заказа)

totalAmount (общая сумма заказа).

Product должна содержать следующие поля:
...
quantity (количество доступных продуктов).
Репозитории:

Создайте интерфейсы репозиториев для сущностей Customer, Order, и Product с использованием Spring Data.
Сервис:

Создайте сервисный класс для управления операциями CRUD (создание, чтение, обновление, удаление) заказов и продуктов. Реализуйте метод для размещения заказа (placeOrder), который будет создавать заказ и вычитать соответствующие суммы из баланса заказчика и списывать продукты из инвентаря. Используйте вложенные транзакции для обеспечения атомарности операций внутри метода размещения заказа.
Транзакции:

Используйте аннотацию @Transactional для управления внешней транзакцией в методе размещения заказа, а также явно настройте внутренние транзакции для обеспечения консистентности данных (если произошла ошибка то должен удалиться заказ а также количество предметов на складе должно откатываться)
REST-контроллер:

Создайте REST-контроллер для обработки запросов, связанных с управлением заказами, продуктами и пользователями. Реализуйте конечные точки для размещения заказа, получения информации о заказах и балансе пользователя.